...
import com.invoice.fegin.CustomsPaymentBookServiceFegin;
...
@Api(value="海关缴款接口",description="海关缴款接口" )
public class CustomsPaymentBookController {

    @Autowired
	private CustomsPaymentBookServiceFegin service;


    

    @ApiOperation(value = "海关缴款分页列表",notes="根据条件查询数据并分页")
    @ApiImplicitParams({
	...
	public  Pages<List<CustomsPaymentBook>> pages( @RequestParam Integer pageSize,@RequestParam Integer pageNum,@ApiParam(name="businessMatters",value="查询条件",required=true) 
	            @RequestBody(required=false) CustomsPaymentBook businessMatters){
		return  service.pages(businessMatters,pageSize,pageNum);
	}
    
    
    @PostMapping("save")
	public  Integer save(@RequestBody CustomsPaymentBook businessMatters){
		return  service.save(businessMatters);
	}

    
    @GetMapping("info/{id}")
	public CustomsPaymentBook info(@PathVariable("id") Long id,CustomsPaymentBook businessMatters){
    ...
    @GetMapping("/state/{id}")
	public  Integer changeDelStateById(@PathVariable("id") Long id,CustomsPaymentBook businessMatters){
    	businessMatters.setId(id);
		return  service.changeDelStateById(businessMatters);
	}
    
    @ApiOperation(value = "海关缴款列表",notes="海关缴款列表")
    @PostMapping(value = "queryList",produces = MediaType.APPLICATION_JSON_VALUE)
	public  List<CustomsPaymentBook> queryList( @ApiParam(name="businessMatters",value="查询条件",required=false) 
	            @RequestBody(required=false) CustomsPaymentBook businessMatters){
